import React from 'react';
import { storiesOf } from '@storybook/react';
import centered from '@storybook/addon-centered/react';
import Button from './index';

const negativeStyle = {
	backgroundColor: '#274247',
	padding: '100px',
	display: 'flex',
};

storiesOf('Button', module).addDecorator(centered)
	.add('Primary', () => (
		<div style={{ display: 'flex' }}>
			<Button primary>Primary button</Button>
			<Button icon={<i className="feather-chevron-down" />} primary>Primary icon</Button>
			<Button primary disabled>Disabled button</Button>
		</div>
	))
	.add('Secondary', () => (
		<div style={{ display: 'flex' }}>
			<Button>Secondary button</Button>
			<Button icon={<i className="feather-chevron-down" />}>Secondary icon</Button>
			<Button disabled>Disabled button</Button>
		</div>
	))
	.add('Primary - Negative', () => (
		<div style={negativeStyle}>
			<Button negative primary>Primary button</Button>
			<Button icon={<i className="feather-chevron-down" />} negative primary>Primary icon</Button>
			<Button negative primary disabled>Disabled button</Button>
		</div>
	))
	.add('Secondary - Negative', () => (
		<div style={negativeStyle}>
			<Button negative>Secondary button</Button>
			<Button icon={<i className="feather-chevron-down" />} negative>Secondary icon</Button>
			<Button negative disabled>Disabled button</Button>
		</div>
	));
